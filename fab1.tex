...
This section gives an overview of the objects used for the observables based on subjet-assisted tracks, which are the large-radius jet mass, the Energy Correlation Functions and the n-subjettiness, as they are used by default within ATLAS.
...
The \textit{calorimeter mass} or $\mcal$ is a widely used variable which takes as input the topo-cluster information. Given that each topo-cluster $i$ has a 3D information on the energy deposit, $E_i$, $\eta$ and $\phi$, the mass can be simply calculated from 4-vector properties:
...
The mass $m^\textrm{trk}$ is then calculated summing up the 4-momenta of those tracks which passed the selection and are ghost associated to the groomed jet.
...
% Given the set of tracks which pass this selection, the mass $m^\textrm{trk}$ is calculated summing up the 4-momenta of those tracks which are ghost associated to the groomed jet.
...
% One could then think about creating a variable which exploits the advantages of both and minimizes the disadvantages. As seen, the track mass is missing the neutral component, i.e. each measurement is missing the fraction $\frac{neutral+charged}{charged}$, but it could be corrected on a jet-by-jet basis: this leads to the definition of the \textit{track-assisted mass} ($\mta$):
% \begin{equation}
%  \mta=\frac{p_T^\textrm{calo}}{p_T^\textrm{trk}}\times m^\textrm{trk}
% \end{equation}

% It can be intuitively understood as follows: the term $m^\textrm{trk}$ has the superior angular resolution, but misses the neutral component; the ratio $p_T^\textrm{calo}/p_T^\textrm{trk}$, representing exactly the $(neutral+charged)/charged$ ratio, ``restores'' the correct value of the mass back to $charged+neutral$.
% \begin{figure}[!ht]
%   \centering
...
%   \caption[$\mcal$ and $\mta$ comparison for $W/Z$ jets and top jets]{The comparison between the performance of $\mcal$ and $\mta$ for $W/Z$ jets (left) and top jets (right); on the x-axis the transverse momentum and on the y-axes the $\iqr$ of the mass distribution, from \cite{art35}. A better observable has lower values on the y-axis. }
%   \label{fig:mta3}
% \end{figure}
...
%   \caption[Performance of the $\mta$ with the boosted Higgs sample]{Performance of the $\mta$ with the boosted Higgs sample; the $\mta$ is the blue line, the $\mcomb$ will be described later in this Section. From \cite{art39}. The FoM here is the resolution of the Response.}
%   \label{fig:mta4}
% \end{figure}
...
Getting inspired from the formula $\mta=p_T^\textrm{calo}/p_T^\textrm{trk}\times m^\textrm{trk}$, we would like to replicate this at subjet level, i.e.
...
$$p_\mu^{TJ} = \spvec{m^{TJ};p_T^{TJ};\eta^{TJ};\phi^{TJ}} \to p_\mu^\textrm{TA}=\spvec{m^{TJ}\times\frac{p_T^{SJ}}{p_T^{TJ}} ;p_T^{SJ};\eta^{TJ};\phi^{TJ}} $$
 
where $p_\mu^\textrm{TA}$ is the track-assisted subjet's 4-vector. If we label $i$ the $i$-th track-jet of the $N$ ones present in the large-$R$ jet,

$$ \mtas=\sqrt{\left(\sum_i^N p^\textrm{TA} \right)_\mu \left(\sum_i^N p^\textrm{TA} \right)^{_\mu}} $$
...
$$p_\mu^\textrm{trk} = \spvec{m^\textrm{trk};p_T^\textrm{trk};\eta^\textrm{trk};\phi^\textrm{trk}} \to p_\mu^\textrm{TA}=\spvec{m^\textrm{trk} ;p_T^\textrm{trk} \times\frac{p_T^{SJ}}{p_T^{TJ}};\eta^\textrm{trk};\phi^\textrm{trk}}$$
...
$$ \mtas=\sqrt{\left(\sum_i^M p^\textrm{TA} \right)_\mu \left(\sum_i^M p^\textrm{TA} \right)^{_\mu}} $$
...
This is discussed in great details in Ref.~\cite{art35}.
...
\begin{equation}\begin{split}
 ...
 a=\frac{\sigma_\textrm{calo}^{-2}}{\sigma_\textrm{calo}^{-2}+\sigma_\textrm{TA}^{-2}} \qquad b=\frac{\sigma_\textrm{TA}^{-2}}{\sigma_\textrm{calo}^{-2}+\sigma_\textrm{TA}^{-2}}
\end{split}
\label{eq:mtacomb}
\end{equation}
where $\sigma_\textrm{calo}$ and $\sigma_\textrm{TA}$ are the $\mcal$'s and $\mta$'s resolution functions. The $\mcomb$ then is the $\mta-\mcal$ combination.
